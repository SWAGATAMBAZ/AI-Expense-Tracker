...
- **Phase 4 — Dashboard**: date-range filter component, total spend, category breakdown (Recharts), upcoming spend list, savings forecast (PRD §9/10 formula, floor at ₹0), payment-method donut chart. The dashboard's quick actions (Transactions, Add recurring expense, AI feature, Add expense, Account) were later consolidated into a persistent bottom navigation bar per an approved wireframe, replacing a floating AI button. The AI slot opens a placeholder page for now — it gets real behavior in Phases 5/6 below, not this phase.
